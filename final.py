...
voice = wave.open("./data/我和你.wav", "rb")
voice_with_noise = wave.open("./data/我和你加噪声.wav", "rb")
...
targetPath = './'
if SAVEFIG is True:
	if not os.path.exists(targetPath+'outputImg'):
		os.mkdir(targetPath+'outputImg')
	targetPath += 'outputImg/'
...
for v in [voice, voice_with_noise]:
	...
	#plot freq domain
	freq = np.linspace(0, framerate/2, int(nframes/2))
	plt.figure()
	...
	if [voice, voice_with_noise].index(v) == 1:
		#iir and plot
		...
		'''
		#waveIir = lowpass(waveFFT, nframes)
		waveIir = iirLowPass(waveData, 2500, 3500, 3, 40, framerate)
		print('iir'+str(waveIir.shape))
		...
		plt.xlabel("Time/s")
		plt.ylabel("NormedAmplitude")
		plt.title('voice_with_noise_iir_timedomain')
		plt.grid()
		if SAVEFIG is True:
